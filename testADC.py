...
def main(args):
	...
	# Consider scrapping the if statement as data is read correctly regardless if DRDY pin is ready
		# likely need a clock source for MCLK for DRDY to operate correctly

	#if ad7705.dataReady(CHN_AIN1):
		#time.sleep(2)
	#	print "%s" %ad7705.dataReady(CHN_AIN1)
	print "raw %s" % ad7705.readADResultRaw(CHN_AIN1) 
        	#time.sleep (1.5)
	print "voltage %s" % ad7705.readVoltage(CHN_AIN1,2.5)
		#time.sleep (1.5)
	#else:
	#	print "data register not ready..waiting.."
	#	print "%s" %ad7705.dataReady(CHN_AIN1)
	#	print "voltage %s" % ad7705.readVoltage(CHN_AIN1,2.5)
	time.sleep (1)
